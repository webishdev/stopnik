...
func TestUserHandling(t *testing.T) {
	...
	assertUserDoesNotExistWithName(t, "wrong", config)
	assertUserDoesNotExistWithName(t, "empty", config)
	assertUserDoesNotExistWithName(t, "", config)
}
...
func TestClientHandling(t *testing.T) {
	configLoader := NewConfigLoader(func(filename string) ([]byte, error) {
		...
		*origin = Config{
			Clients: []Client{
				{Id: "foo", Secret: "d82c4eb5261cb9c8aa9855edd67d1bd10482f41529858d925094d173fa662aa91ff39bc5b188615273484021dfb16fd8284cf684ccf0fc795be3aa2fc1e6c181"},
				{
					Id:         "bar",
				...
				{Id: "moo", Secret: "ddaf35a193617abacc417349ae20413112e6fa4e89a97ea20a9eeee64b55d39a2192992a274fc1a836ba3c23a3feebbd454d4423643ce80e2a9ac94fa54ca49f"},
	...
	assertClientDoesNotExistWithId(t, "wrong", config)
	assertClientDoesNotExistWithId(t, "empty", config)
	assertClientDoesNotExistWithId(t, "", config)
...
func assertUserDoesNotExistWithName(t *testing.T, username string, config *Config) {
	_, exists := config.GetUser(username)
	if exists {
		t.Error("expected user")
	}
}
...
func assertClientDoesNotExistWithId(t *testing.T, id string, config *Config) {
	_, exists := config.GetClient(id)
	if exists {
		t.Error("expected client")
	}
}
